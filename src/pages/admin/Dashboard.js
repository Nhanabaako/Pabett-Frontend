...
// ✅ UPDATED — mirrors routes/products.js VALID_CATEGORIES
  ...
  return (
    ...
    >
      <Stack direction="row" alignItems="center" spacing={2}>
        <Box sx={{ bgcolor: `${color}18`, p: 1.5, borderRadius: 2, display: "flex" }}>
          {React.cloneElement(icon, { sx: { color, fontSize: 28 } })}
        </Box>
        <Box>
          <Typography variant="body2" color="text.secondary" fontWeight={500}>{label}</Typography>
          <Typography variant="h4" fontWeight={800} color="#2C3E64">{value}</Typography>
  ...
  return (
    <Paper sx={{ p: 3, borderRadius: 3, height: "100%", boxShadow: "0 4px 16px rgba(0,0,0,0.07)" }}>
      <Stack direction="row" alignItems="center" spacing={1.5} mb={2}>
        ...
        <Typography variant="h6" fontWeight={700} color="#2C3E64">{title}</Typography>
      </Stack>
      <Divider sx={{ mb: 2 }} />
      {rows.length === 0 ? (
        ...
        rows.map((row) => (
          <Stack key={row.key} direction="row" alignItems="center" justifyContent="space-between" mb={1.2}>
            <Stack direction="row" spacing={1} alignItems="center">
              <Box sx={{ width: 10, height: 10, borderRadius: "50%", bgcolor: row.color, flexShrink: 0 }} />
              <Typography variant="body2" color="text.secondary">{row.label}</Typography>
            </Stack>
            <Stack direction="row" spacing={1} alignItems="center">
              <Chip label={row.count} size="small" sx={{ bgcolor: row.color, color: "#fff", fontWeight: 700, minWidth: 32 }} />
              {total > 0 && (
...
export default function Dashboard() {
  ...
  const confirmedBookings = bookings.filter((b) => b.status === "confirmed" || b.confirmed).length;
  ...
  const galleryBreakdown = GALLERY_SECTIONS
    .map((s) => ({
      key: s.value, label: s.label, color: s.color,
  ...
  return (
    <Box>
      <Box mb={4}>
        <Typography variant="h4" fontWeight={800} color="#2C3E64">📊 Dashboard Overview</Typography>
        <Typography variant="body2" color="text.secondary" mt={0.5}>Live snapshot of your store and bookings</Typography>
      </Box>

      <Grid container spacing={3} mb={5}>
        <Grid item xs={12} sm={6} md={3}>
          <StatCard icon={<InventoryIcon />} label="Total Products" value={products.length} color="#00B6AD"
            sub={`${productBreakdown.length} categor${productBreakdown.length !== 1 ? "ies" : "y"}`} />
        </Grid>
        <Grid item xs={12} sm={6} md={3}>
          <StatCard icon={<CollectionsIcon />} label="Gallery Images" value={images.length} color="#9c27b0"
            sub={`${galleryBreakdown.length} section${galleryBreakdown.length !== 1 ? "s" : ""}`} />
        </Grid>
        <Grid item xs={12} sm={6} md={3}>
          <StatCard icon={<EventAvailableIcon />} label="Total Bookings" value={bookings.length} color="#2196f3"
            sub={`${confirmedBookings} confirmed`} />
        </Grid>
        <Grid item xs={12} sm={6} md={3}>
          <StatCard icon={<PendingActionsIcon />} label="Pending Bookings" value={pendingBookings} color="#ff9800"
      ...
      <Grid container spacing={3}>
        <Grid item xs={12} md={6}>
          <BreakdownCard title="Products by Category" icon={<InventoryIcon />} color="#00B6AD"
            rows={productBreakdown} total={products.length} />
        </Grid>
        <Grid item xs={12} md={6}>
          <BreakdownCard title="Gallery by Section" icon={<CollectionsIcon />} color="#9c27b0"
            rows={galleryBreakdown} total={images.length} />
        </Grid>
      </Grid>

      {bookings.length > 0 && (
        <Box mt={5}>
          <Typography variant="h6" fontWeight={700} color="#2C3E64" mb={2}>🗓️ Recent Bookings</Typography>
          <Paper sx={{ borderRadius: 3, overflow: "hidden", boxShadow: "0 4px 16px rgba(0,0,0,0.07)" }}>
            {bookings.slice(0, 5).map((b, i) => (
              <Box key={b._id || i}>
                ...
                >
                  <Box>
                    <Typography fontWeight={700} color="#2C3E64">{b.fullName}</Typography>
                  ...
                  <Stack direction="row" spacing={1} alignItems="center">
                    <Chip
                      ...
                      size="small"
                      sx={{ bgcolor: b.appointmentType === "on-location" ? "#ff9800" : "#4caf50", color: "#fff", fontWeight: 600 }}
                    />
                    {(b.status || b.confirmed) && (
                      <Chip
                        icon={b.status === "confirmed" || b.confirmed ? <CheckCircleIcon fontSize="small" /> : <PendingActionsIcon fontSize="small" />}
